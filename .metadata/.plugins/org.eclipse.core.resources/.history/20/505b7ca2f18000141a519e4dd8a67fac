...
COMPLEX w[(FFT_LENGTH - (ORDER - 1))/RADIX] = {0};
...
void main()
{
	...
	while(1)						// main loop - do nothing but wait for interrupts
	{
		if(pingFlag == 1)
		{
			...
			{
				outputPING[n] = PING[n].re * 32768;
			}
			lflag = 0; //done processing PING
		}
		if(pongFlag == 1)
		{
			for(n = 0; n < (FFT_LENGTH - (ORDER - 1)); n++)
			{
				outputPONG[n] = PONG[n].re * 32768;
			}
			lflag = 1; //done processing PONG
		}
	}
}
...
interrupt void serialPortRcvISR()
{
	...
	if(pingFlag == 1 && lflag == 1) //if ping is being processed, and PONG is done processing...
	{
		//... FILL DAT SHIT UPP
	...
	if(pongFlag == 1 && lflag == 0) //if pong is being processed, and PING is done processing...
	{
		//... FILL DAT SHIT UPPP
		...
		temp.channel[0] = (short)outputPING[pingIndex];
	}
