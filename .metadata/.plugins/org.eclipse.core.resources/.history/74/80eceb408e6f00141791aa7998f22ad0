...
int w[33] = {0};
short NUM_shifted[11][3] = {0};
short DEN_shifted[11][3] = {0};
short n = 0;
...
void main()
{
	...
	for(i = 0; i < L; i++)
	{
		for(j = 0; j < 3; j++)
		{
			NUM_shifted[i][j] = NUM[i][j] << 8;
			DEN_shifted[i][j] = DEN[i][j] << 8;
		}
	}
...
int lordBiquad(int x, int row)
{
	...
	w[(row*3)+n] = x - DEN_shifted[row][1]*w[(row*3)+n1] - DEN_shifted[row][2]*w[(row*3)+n2];

	//compute the current output of this section of the filter
	y = NUM_shifted[row][0]*w[(row*3)+n3] + NUM_shifted[row][1]*w[(row*3)+n1] + NUM_shifted[row][2]*w[(row*3)+n2];
