...
volatile float outputPONG[FFT_LENGTH - (ORDER - 1)] = {0};
volatile float outputPING[FFT_LENGTH - (ORDER - 1)] = {0};
...
//				V[n] = cmplxMult(h[n], pingU[n]);
				V[n].re = (h[n].re * pingU[n].re) - (h[n].im * pingU[n].im);
				V[n].im = (h[n].im * pingU[n].re) + (h[n].re * pingU[n].im);
...
//				V[n] = pingU[n];
		...
		else if(pongFlag == 1 && lflag == 0)
		{
			...
			for(n = 0; n < FFT_LENGTH; n++)
			{
				V[n].re = (h[n].re * pongU[n].re) - (h[n].im * pongU[n].im);
				V[n].im = (h[n].im * pongU[n].re) + (h[n].re * pongU[n].im);
